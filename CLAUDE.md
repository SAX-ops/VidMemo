...
uv run pytest tests/test_summarizer.py tests/test_summary_cache.py tests/test_summary_router.py  # AI summary tests
```
...
- **`services/bilibili.py`** — Bilibili subtitle extraction via direct `api.bilibili.com/x/web-interface/view` + `x/v2/dm/view` calls (used by `SubtitleExtractor` for CC subtitle priority). Independent from yt-dlp's Bilibili extractor.
- **`services/summarizer.py`** — AI video summary module. Two-stage LLM pipeline: Stage 1 (`SUMMARY_MODEL`, default `mimo-v2.5`) generates chapter outlines via semantic segmentation (TF-IDF cosine similarity) + LLM summarization. Stage 2 (`EXECUTIVE_SUMMARY_MODEL`, default `mimo-v2-flash`) generates executive summary (core topic, key insights, author conclusion) with quality validation and retry logic. **A second Stage 2 sibling** (`MINDMAP_MODEL`, falls back to `EXECUTIVE_SUMMARY_MODEL` → `SUMMARY_MODEL`) generates the hierarchical mindmap JSON (`{root, children:[{title, timestamp, children:[...]}]}`) with the same retry + validation pattern. Both Stage 2 calls run in parallel and either may return `None` on quality-gate failure without affecting the outline or the other branch. **Chat with Video** (`CHAT_MODEL`, falls back to `EXECUTIVE_SUMMARY_MODEL` → `SUMMARY_MODEL`): character n-gram retrieval (`_retrieve_by_chapter`) with per-chapter diversification (max 3 segments/chapter, chapter score = max_score + 0.1×hit_count), prompt assembly (`_build_chat_prompt`, ≤3000 chars), LLM streaming with `[[CH_N]]` citation protocol (0-based, valid-chapters restricted), and citation parsing (`_parse_chat_citations`). Empty retrieval returns fixed error without calling LLM. Key functions: `SubtitleExtractor.extract()`, `_semantic_segment()`, `parse_outline_json()`, `generate_executive_summary()`, `parse_executive_summary()`, `generate_mindmap()`, `parse_mindmap()`, `_retrieve_by_chapter()`, `_build_chat_prompt()`, `_parse_chat_citations()`, `generate_chat_answer()`. Config: `OPENAI_API_KEY`, `SUMMARY_BASE_URL`, `SUMMARY_MODEL`, `EXECUTIVE_SUMMARY_MODEL`, `EXECUTIVE_SUMMARY_TIMEOUT`, `MINDMAP_MODEL`, `MINDMAP_TIMEOUT`, `CHAT_MODEL`, `CHAT_TIMEOUT`.
- **`services/summary_cache.py`** — File-based summary cache (JSON, 30-day TTL). Stores `summary_md`, `outline`, `executive_summary`, `mindmap`, `subtitle_meta`.
- **`routers/summary.py`** — `POST /api/summarize` SSE endpoint (events: `cache_hit`, `subtitle`, `summary`, `outline`, `summary_md`, `executive_summary`, `mindmap`, `done`, `error`). `POST /api/chat` SSE endpoint (events: `chat_token`, `chat_done`, `chat_error`). Chat reads cached subtitle/outline from `summary_cache.json`, retrieves relevant segments, and streams an LLM answer with chapter-level citations. No cache → `chat_error(no_cache)`. No subtitles → `chat_error(no_subtitle)`.
...
- **`components/`** — `DownloadInput.vue` (URL input with dynamic button text "解析视频"/"解析中..." + loading spinner), `VideoPreview.vue` (custom video player with DASH audio sync, dynamic quality selector, download button), `VideoSummary.vue` (AI summary panel: executive summary + chapter outline + mindmap tab + chat tab, SSE-driven with skeleton loading), `MindMap.vue` (Mind-Elixir renderer for the hierarchical mindmap, emits `node-click` for video seek), `ChatPanel.vue` (video Q&A panel: streaming answer via SSE, chapter-level citation cards with click-to-seek, stop button, empty/error states), `QualitySelector.vue` (dropdown populated from `/api/parse` formats list — no hardcoded quality keys), `ProgressTracker.vue` (progress bar + status), `PlatformList.vue` (supported platforms display)
...
- **`composables/useSSE.ts`** — SSE client for the `/api/summarize` stream. Returns `{abort}` so the parent component can cancel on unmount/re-click (prevents continued LLM billing after the user navigates away).
...
| POST | `/api/summarize` | SSE stream of AI video summary. Events: `cache_hit` → `subtitle` → `summary` tokens → `outline` → `summary_md` → `executive_summary` → `mindmap` → `done` (or `error`). `executive_summary` and `mindmap` run in parallel; either may be absent on quality-gate failure. Body: `{url, language}`. |
| POST | `/api/chat` | Chat with Video — SSE stream. Reads cached subtitle/outline, retrieves relevant segments via character n-gram, streams LLM answer with `[[CH_N]]` citation markers. Events: `chat_token` (逐 token) → `chat_done` (citations only) or `chat_error` (code: `no_cache`/`no_subtitle`/`no_results`/`timeout`/`llm_error`). Body: `{url, question}`. Empty retrieval returns `chat_error` without calling LLM. |
...
- **AI summary SSE flow**: `POST /api/summarize` → cache lookup → subtitle extraction (yt-dlp or bilibili DM API) → Stage 1 LLM (chapters) → Stage 2 LLM **branches in parallel** (executive summary + mindmap) → cache write → done. If either Stage 2 branch fails quality validation (executive: banned patterns/length; mindmap: missing root/wrong chapter count/parse failure), the matching SSE event is **skipped** (frontend hides the section rather than show low-quality content). The mindmap LLM returns strict JSON only — no Mermaid, no markdown; timestamps are always grafted server-side from the outline (chapters use their own timestamp; leaves inherit the parent chapter's). Set `EXECUTIVE_SUMMARY_MODEL=mimo-v2-flash` — `mimo-v2.5` returns empty responses consistently. `MINDMAP_MODEL` falls back to `EXECUTIVE_SUMMARY_MODEL` if unset.

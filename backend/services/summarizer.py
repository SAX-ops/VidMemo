...
## 章节时间戳
（在 markdown 结束后另起一行，输出 JSON 代码块，章节顺序与时间线一致）

```json
{{"chapters": [{{"time": 0, "title": "章节一标题"}}, {{"time": 120, "title": "章节二标题"}}]}}
```
```

## 硬性要求

- 主章节必须用 `## ` 二级标题，**不要**写成编号列表（不要写 `1. 视频概述`、`### 1. 视频概述`）
- 内容大纲里的子章节必须用 `### ` 三级标题
- 核心知识要点必须用 markdown 编号列表（`1. xxx`）
- 章节时间戳的 JSON 必须能被 `json.loads` 解析：
  - 外层必须是完整 `{{"chapters": [...]}}`，**不要**漏写开头的 `{{`
  - `time` 是整数（不加引号），`title` 是字符串（加双引号）
  - 无尾逗号、无注释
- 章节数量 = 内容大纲里的 `### ` 数量
- 章节 title 必须与内容大纲里的 `### ` 标题**完全一致**

## 常见错误（请避免）

- ❌ `1. 视频概述` → ✅ `## 视频概述`
- ❌ `chapters": [...]` （缺外层 `{{`）→ ✅ `{{"chapters": [...]}}`
- ❌ `### 1. 视频概述` → ✅ `## 视频概述`
- ❌ 把章节时间戳直接接到标题文字后面 → ✅ 用 ```json ``` 代码块包裹
